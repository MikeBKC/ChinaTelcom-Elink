...
static int aes_encrypt_cbc (lua_State *L) {
	unsigned char buffer[2048]={0};
  uint8_t iv[16]  = {0};
  size_t inl, secretl, outl;
  const char *in = luaL_checklstring(L, 1, &inl);
  const char *secret = luaL_checklstring(L, 2, &secretl);

    unsigned char key[]={0xd1, 0x7b ,0xdb ,0xf5 ,0x41 ,0x15 ,0x97 ,0x25, 0x2b ,0xee ,0x2b ,0x88 ,0xcf ,0x07 ,0x99 ,0x05};
...
memset(iv,0,16);
				...
				if ((slen + 1) % BLOCKLEN == 0) {  
								len = slen + 1;  
				} else {  
								len = ((slen + 1) /BLOCKLEN  + 1) * BLOCKLEN;  
				}  
				...
				strncpy((char*)input_string, in, slen);  


  	AES_CBC_encrypt_buffer(buffer, input_string, len, secret, iv);
//	  AES_CBC_encrypt_buffer(buffer, input_string, len, key, iv);
...
static int aes_decrypt_cbc (lua_State *L) {
				char buffer[2048]={0};
				uint8_t iv[16]  = {0};
				size_t inl, secretl, outl;
				const char *in = luaL_checklstring(L, 1, &inl);
				...
				AES_CBC_decrypt_buffer(buffer, in, inl, secret, iv);
				//AES_CBC_decrypt_buffer(buffer, in, inl, key, iv);
